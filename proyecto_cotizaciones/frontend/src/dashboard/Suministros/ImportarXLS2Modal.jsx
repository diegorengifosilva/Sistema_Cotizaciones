...
import plantillaXLS from "@/assets/PlantillasXLS/Plantilla002.xls";
import * as XLSX from "xlsx";
import { FileUp } from "lucide-react";

export default function ImportarXLS2Modal({ open, onClose, onSelectFile }) {
  const fileInputRef = useRef(null);
  const [archivoSeleccionado, setArchivoSeleccionado] = useState(null);
  const handleDescargarPlantilla = () => {
    const link = document.createElement("a");
    link.href = plantillaXLS;
    link.download = "Plantilla_Suministros_002.xls";
        ...
        <div className="p-2">
          <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-6 shadow-inner">
            ...
            <div className="grid grid-cols-2 gap-6 items-center">
              ...
              {/* ZONA DE IMAGEN/GIF */}
              <div className="flex flex-col items-center justify-center bg-white rounded-2xl p-3 border border-slate-200 shadow-sm group">
                <img
                  src={archivoSeleccionado ? importarXlsGif : importarXlsImg}
                  alt="Importar XLS"
                  className={`
                    max-h-32 object-contain transition-all duration-300
                    ${!archivoSeleccionado ? "cursor-pointer group-hover:scale-110" : "scale-95 opacity-90"}
                  `}
                  onClick={() => {
                ...
                />
                {!archivoSeleccionado && (
                  <span className="text-[9px] text-slate-400 font-black uppercase mt-2 tracking-tighter">
                    Descargar Plantilla
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-3">
                <Button
                  ...
                  className="text-[11px] font-black uppercase tracking-widest text-cyan-700 bg-cyan-50/50 hover:bg-cyan-100 border border-cyan-200 rounded-xl h-14 transition-all shadow-sm leading-tight text-center"
                >
            ...
            {/* STATUS DE CARGA (Aparece cuando hay archivo) */}
            {archivoSeleccionado && (
              <div className="mt-4 px-3 py-2 bg-emerald-50 border border-emerald-100 rounded-xl flex items-center gap-2 animate-in fade-in slide-in-from-top-1">
                <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                <span className="text-[10px] font-bold text-emerald-700 truncate uppercase tracking-tight">
                  Procesando: {archivoSeleccionado.name}
                </span>
              </div>
            )}
          </div>
